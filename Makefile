...
CFLAGSCPX = -DIL_STD -O -DNDEBUG -I$(CPOPTIMIZERDIR)/include -I$(CONCERTDIR)/include -I$(CPLEXDIR)/include  -lconcert -lilocplex -lcplex -lm -ldl -fPIC -fstrict-aliasing -pedantic -Wall -fexceptions -frounding-math -Wno-long-long -m64 -DILOUSEMT -D_REENTRANT -DILM_REENTRANT -fno-strict-aliasing

LDFLAGSCPX = -L$(CPOPTIMIZERDIR)/lib/x86-64_linux/static_pic -lcp -L$(CPLEXDIR)/lib/x86-64_linux/static_pic -L$(CONCERTDIR)/lib/x86-64_linux/static_pic -lconcert -lilocplex -lcplex -lpthread -lm -ldl -fno-strict-aliasing
...
StochProg.so: 
	g++ $(CFLAGS) $(INCL) $(CLINKFLAGS) -shared -fPIC *.cpp *.h -o StochProg.so $(LIBS) $(ADDLIBS) -L${MKLROOT}/lib/intel64 -Wl,--no-as-needed -lmkl_intel_lp64 -lmkl_lapack95_lp64 -lmkl_sequential -lmkl_core -lgfortran -fopenmp -lpthread -lmkl_mc -lmkl_def -lm -ldl 

StochProg: 
	g++ $(CFLAGS) $(INCL) $(CLINKFLAGS) *.cpp *.h -o StochProg -L${MKLROOT}/lib/intel64 -Wl,--no-as-needed -lmkl_intel_lp64 -lmkl_lapack95_lp64 -lmkl_sequential -lmkl_core -lgfortran -fopenmp -lpthread -lm -lmkl_mc -lmkl_def -ldl $(LIBS) $(ADDLIBS)
